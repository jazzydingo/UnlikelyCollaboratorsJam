...
namespace game 
{
    public class InventoryController : MonoBehaviour
    {
        ...
        {
            index = 0;
            activeSlot = inventoryArray[index];
        ...
        void UseSelected()
        {
            activeObj = objArray[index];
            if (Input.GetKeyUp(KeyCode.F) && activeObj != null)
            {
                
                activeObj.gameObject.GetComponent<Interactable>().UseObject();
            }
        }
        ...
        void HighlightCurrent()
        {
            ...
            highlight.transform.position = Vector3.Lerp(highlight.transform.position, activeSlot.transform.position, 10f * Time.deltaTime);
        }
